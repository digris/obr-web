...
class AppleIdLoginView(
    ...
    @extend_schema(
        methods=["POST"],
        parameters=[
            serializers.AppleIdLoginSerializer,
        ],
        request=None,
        responses={
            200: serializers.UserSerializer,
        ...
        operation_id="apple_id_token_login",
        auth=[],
        description="""Login user by apple ID-Token. (native flow in obr-app)
        Responds `200` for existing and `201` for created user.""",
        tags=["authentication"],
    )
    def post(request):
        request_serializer = serializers.AppleIdLoginSerializer(data=request.data)
        ...
        authorization_code = request_serializer.validated_data["authorization_code"]
        profile = request_serializer.validated_data["profile"]

        try:
            user, user_created = apple_id_login.get_or_create_user(
                request=request,
                id_token=id_token,
                authorization_code=authorization_code,
                profile=profile,
            )
        except apple_id_login.AppleIdLoginError as e:
            return Response(
                {
                    "message": f"{e}",
                },
...
class GoogleOneTapLoginView(
    ...
    @extend_schema(
        methods=["POST"],
        parameters=[
            serializers.GoogleOneTapLoginSerializer,
        ],
        request=None,
        responses={
            200: serializers.UserSerializer,
        ...
        operation_id="google_one_tap_login",
        auth=[],
        description="""Login user via Google One-Tap.
        Responds `200` for existing and `201` for created user.""",
        tags=["authentication"],
    )
    def post(request):
        request_serializer = serializers.GoogleOneTapLoginSerializer(data=request.data)
        if not request_serializer.is_valid():
            return Response(
                {
                    "message": request_serializer.errors,
        ...
        try:
            user, user_created = google_one_tap_login.get_or_create_user(
                request=request,
                credential=credential,
            )
        except google_one_tap_login.OneTapLoginError as e:
            return Response(
                {
                    "message": f"{e}",
                },
