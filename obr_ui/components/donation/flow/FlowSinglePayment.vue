...
class PaymentFlow {
  ...
  public async init(): Promise<void> {
    ...
    // NOTE: link is used to collect e-mail address - we only need this for anonymous users
    if (!this.user || !this.user?.uid) {
      await this.mountLink();
    }
  ...
  private async mountLink(): Promise<void> {
    const linkEl = this.elements.create("linkAuthentication", {});
    linkEl.mount("#link");
  }
  ...
  private async mountExpressCheckout(): Promise<void> {
    ...
    expressEl.on("confirm", async (event) => {
      console.debug("expressEl:confirm", event);
...
onMounted(async () => {
  ...
  flow = new PaymentFlow(emit, errors, formEl.value, user.value, props.amount, props.currency);
  await flow.init();
});
